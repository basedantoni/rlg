...
import { insertUserSchema, NewUserParams, UpdateUserParams, updateUserSchema, UserId, userIdSchema, users } from "@/db/schema/users";
...
export const updateUser = async (id: UserId, user: UpdateUserParams) => {
        ...
        try {
                await db
                        .update(users)
                        .set({ ...newUser, updatedAt: new Date().toUTCString() })
